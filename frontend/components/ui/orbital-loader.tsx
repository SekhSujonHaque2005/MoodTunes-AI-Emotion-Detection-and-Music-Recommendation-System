  ...
  return (
    <div className={cn(orbitalLoaderVariants({ messagePlacement }))}>
      <div className={cn("relative w-16 h-16", className)} {...props}>
        ...
        />
        <motion.div
          className="absolute inset-2 border-2 border-transparent border-t-white rounded-full opacity-70"
          animate={{ rotate: -360 }}
          transition={{
            duration: 1.5,
            repeat: Number.POSITIVE_INFINITY,
        ...
        />
        <motion.div
          className="absolute inset-4 border-2 border-transparent border-t-white rounded-full opacity-40"
          animate={{ rotate: 360 }}
          transition={{
            duration: 0.8,
            repeat: Number.POSITIVE_INFINITY,
      ...
      {message && <div className="text-white font-medium tracking-tight animate-pulse">{message}</div>}
    </div>
  )
}
